...
### 1.5 Экспорт в GIF-анимацию

Начиная с версии с GIF-поддержкой, VM может записывать анимированный GIF при включённом захвате кадров. Кадры захватываются после каждой инструкции `DRAW` или `CLS` (максимум 500 кадров).

- `Vm::new_with_capture(code)` — создаёт VM с включённым захватом
- `Vm::write_gif(path)` — записывает захваченные кадры в анимированный GIF
- CLI: `tbvm gif <file>` — выполняет байткод и создаёт `output.gif`
- Telegram бот: команда `/gif <код>` — возвращает GIF-анимацию

Задержка между кадрами: 10 cs (100 мс) при ≤ 100 кадров, 5 cs (50 мс) при большем количестве. Цикл повторения: бесконечный (`Repeat::Infinite`).
